...
def main():
    ...
    with mrcfile.new(output_mrc_prefix+".mrc", overwrite=True) as outMrc:
        ...
        if args.shift_origin:
            outMrc.header.origin.x = -(outData.shape[2] * voxel_size.x)/2
            outMrc.header.origin.y = -(outData.shape[1] * voxel_size.y)/2
            outMrc.header.origin.z = -(outData.shape[0] * voxel_size.z)/2
        else:
